...
class STDiT3Block(nn.Module):
    ...
    def forward(
        ...
        if self.fwd_id < b:
            ...
            min = 15
            max = 28
        if self.fwd_id >= 5 and self.fwd_id <= 25 and self.fwd_id % 3 != 0 and self.block_id >= min and self.block_id <= max:
            if self.block_id == max and self.temporal:
                x = x + delta
            return x
        else:
            ...
            if self.block_id == min and self.temporal:
                delta = x
            if self.block_id == max and self.temporal:
                delta = x - delta
