...
def main():
    ...
    if prompts is None:
        if cfg.get("prompt_path", None) is not None:
            logger.info("Loading prompts from %s", cfg.prompt_path)
    ...
    for i in progress_wrap(range(0, len(prompts), batch_size)):
        ...
        for k in range(num_sample):
            ...
            for loop_i in range(loop):
                ...
                # print(f"Samples shape: {samples.shape} loop_i: {loop_i} save_dir: {save_dir}")
                save_path = f"{save_dir}/{i}.pt"
                # exit(0)
                torch.save(samples, save_path)
                print(f"Saving {prompts[i]} in path: {save_path}")
